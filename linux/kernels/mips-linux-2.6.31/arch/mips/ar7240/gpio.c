...
static u_int32_t loaddefault_flag=0;
struct timer_list  os_timer_t;
struct timer_list  os_timer_t_reboot;
                                        ...
                                        unsigned long count, void *data)
{
	...
	else if (val == 881)  /* cancel reboot */
	{
		reboot_flag = 0 ;
		OS_CANCEL_TIMER(&os_timer_t_reboot);
	} 
	else if (val == 882)  /* reboot now */
	{
		reboot_flag = 48 ;
		printk("\r\nreboot now--");
		OS_INIT_TIMER(NULL, &os_timer_t_reboot, reboot_system_our, &os_timer_t_reboot);
