...
static int getwanconfig(hl2mp_wan_config_t* hl2mp_wan_config, unsigned char wannum )
{
    ...
    int intVal=0;
    printf("\r\nwan num=%d",wannum);
    sprintf(buff_name,"Connect%dName",wannum);
    //todo
    /*
    if( nvram_get(buff_name)!= NULL )
   {
        hl2mp_wan_config->VLANID = (unsigned short)atoi(nvram_get(buff_name));
        sprintf(buff_name,"wan%dports",wannum);
        *(unsigned int *)(hl2mp_wan_config->PORTMASK)=atoi(nvram_get(buff_name));
        ...
        {   
            ...
            snprintf(buff_name, 34, "%s", nvram_get("wan0_dns"));
            if((p = strchr(buff_name, ' ')) != NULL)
            {
                *p='\0';
                *(unsigned int*)hl2mp_wan_config->DNS = inet_addr(buff_name);
                if(*(p+1) != "")
                *(unsigned int*)hl2mp_wan_config->SDNS = inet_addr(p+1);
            }
            else
            {
                *(unsigned int*)hl2mp_wan_config->DNS = inet_addr(buff_name);
            ...
            *(unsigned int*)hl2mp_wan_config->IPADDR = inet_addr(nvram_get("wan0_ipaddr"));
            *(unsigned int*)hl2mp_wan_config->NETMASK = inet_addr(nvram_get("wan0_netmask"));
            *(unsigned int*)hl2mp_wan_config->GATEWAY = inet_addr(nvram_get("wan0_gateway"));

            snprintf(hl2mp_wan_config->PPPEXT, 8, "%s", nvram_get("wan_pppoe_service"));
            snprintf(hl2mp_wan_config->PPPPWD, 16, "%s", nvram_get("wan0_pppoe_username"));
            snprintf(hl2mp_wan_config->PPPUSR, 32, "%s", nvram_get("wan0_pppoe_passwd"));
           
            if(nvram_get("wan_pppoe_mtu") != NULL)
            hl2mp_wan_config->MTU = (unsigned short)atoi(nvram_get("wan_pppoe_mtu"));
          
        }      
   }*/
     if( *nvram_bufget(RT2860_NVRAM,buff_name) == 'c')
   {
   	printf("\r\nname=%s",nvram_bufget(RT2860_NVRAM,buff_name));
   	 sprintf(buff_name,"Connect%dvlanid",wannum);
        hl2mp_wan_config->VLANID = htons_mme(atoi(nvram_bufget(RT2860_NVRAM,buff_name)));
	hl2mp_wan_config->WANID=wannum;
        sprintf(buff_name,"wan%dports",wannum);
        //*(unsigned int *)(hl2mp_wan_config->PORTMASK)=atoi(nvram_get(buff_name));
        sprintf(buff_name,"Connect%dethports",wannum);
	strcpy(tmpBuf,nvram_bufget(RT2860_NVRAM,buff_name));
	if(strchr(tmpBuf,'3') != NULL)
	*(hl2mp_wan_config->PORTMASK)=*(hl2mp_wan_config->PORTMASK) | 0x01;	
	 //*(unsigned int *)(hl2mp_wan_config->PORTMASK)=0X0303;
	 sprintf(buff_name,"Connect%dwifiports",wannum);
	strcpy(tmpBuf,nvram_bufget(RT2860_NVRAM,buff_name));
	printf("\r\neifiports=%s--vlanid=%x",tmpBuf, hl2mp_wan_config->VLANID);
	if(strchr(tmpBuf,'0') != NULL)
		 *(hl2mp_wan_config->PORTMASK+1)=*(hl2mp_wan_config->PORTMASK+1) | 0x01;
	if(strchr(tmpBuf,'1') != NULL)
		 *(hl2mp_wan_config->PORTMASK+1)=*(hl2mp_wan_config->PORTMASK+1) | 0x02;
	if(strchr(tmpBuf,'2') != NULL)
		 *(hl2mp_wan_config->PORTMASK+1)=*(hl2mp_wan_config->PORTMASK+1) | 0x04;
	if(strchr(tmpBuf,'3') != NULL)
		 *(hl2mp_wan_config->PORTMASK+1)=*(hl2mp_wan_config->PORTMASK+1) | 0x08;
        sprintf(buff_name,"Connect%dOperationMode",wannum);
	 intVal=atoi(nvram_bufget(RT2860_NVRAM,buff_name));
	if(intVal)
       	 hl2mp_wan_config->WANMODE =0;
	else
		hl2mp_wan_config->WANMODE =1;
...
{  
    ...
    unsigned int wififlag=0,routeflag=0;
	unsigned char currentRouteNum=0;
    ...
    for(r=271; r<271 + WAN_MAXNUM*WAN_MAXNUM; r++)
    {
        ...
        {  
           ...
           {
                if((r+l) == htons_mme((hl2mp_wan_config+k)->VLANID))
                    break;
           }
            if(l<WAN_MAXNUM)
                break;
    ...
    for(k=0,l=0, s=0, r_c=0; k<num; k++)
    {
        ...
        {
            if(htons_mme((hl2mp_wan_config+k)->VLANID) == 1)
            {
                wanvid_utag[num - 1]=k;
                wanvid_utag_1=1;
            }
 ...
 for(k=0; k < 2; k++)
    {   
        sprintf(buff_name,"Connect%dethports",k+1);
        nvram_unset(buff_name);
        sprintf(buff_name,"Connect%dwifiports",k+1);
        nvram_unset(buff_name);
	 sprintf(buff_value,"");
	 nvram_bufset(RT2860_NVRAM,buff_name,buff_value);
        sprintf(buff_name,"Connect%dvlanid",k+1);
        nvram_unset(buff_name);
        DBG_PRINT("nvram_unset is %s\n",buff_name);
        sprintf(buff_name,"Connect%dOperationMode",k+1);
        nvram_unset(buff_name);
    }


    for(k=0, j=0, i=0, m=1; k < num; k++)
    {   
        if(wanvid_utag[k] < 6)
       {
	...
	wififlag=0;
        ...
        if((wanports & WAN_PORT_MASK)&&(wanvid >= 0)&& ((hl2mp_wan_config+wanvid_utag[k])->OPCODE == 0))
       { 
       	//if((wanvid_utag[k]+1) > 2)
			//return 0;
            wanmode=(hl2mp_wan_config+wanvid_utag[k])->WANMODE;
		if(wanmode)
		{
            		sprintf(buff_value, "%s", "0");
			routeflag=1;
			currentRouteNum++;
		...
		{
			sprintf(buff_value, "%s", "1");
			routeflag=2;
			currentBridgeNum++;
			if(currentBridgeNum > 1)
				continue;
		}
		sprintf(buff_name,"Connect%dOperationMode",routeflag);
		printf("\r\ncon opmode=%s--value=%s\r\n",buff_name,buff_value);
            nvram_bufset(RT2860_NVRAM,buff_name, buff_value);
			
            sprintf(buff_name,"Connect%dvlanid",routeflag);
            sprintf(buff_value,"%d",wanvid);
	     printf("\r\ncon name=%s--value=%s\r\n",buff_name,buff_value);
            nvram_bufset(RT2860_NVRAM,buff_name, buff_value);
		if(wanvid)
			{
			nvram_bufset(RT2860_NVRAM,"Connect1vlanEn", "1");
			nvram_bufset(RT2860_NVRAM,"Connect2vlanEn", "1");
			}
             else
             	{
             		nvram_bufset(RT2860_NVRAM,"Connect1vlanEn", "0");
			nvram_bufset(RT2860_NVRAM,"Connect2vlanEn", "0");
             	}
            sprintf(buff_name,"Connect%dethports",routeflag);
	    if(wanports & 0x01)
           	 sprintf(buff_value,"%s","3");
	    else
		 sprintf(buff_value,"");
	     printf("\r\ncon ethports=%s--value=%s\r\n",buff_name,buff_value);
            nvram_bufset(RT2860_NVRAM,buff_name, buff_value);

            sprintf(buff_name,"Connect%dwifiports",routeflag);
            sprintf(buff_value,"");
            if(wanports & 0x10)
            {
	            	wififlag=1;
	            	sprintf(buff_value,"0");
            }
	    if(wanports & 0x20)
            	{
            		if(wififlag)
            			sprintf(buff_value,"%s;1",buff_value);
			else
				sprintf(buff_value,"1");
			wififlag=1;
		 ...
		 if(wanports & 0x40)
            	{
            		if(wififlag)
            			sprintf(buff_value,"%s;2",buff_value);
			else
				sprintf(buff_value,"2");
			wififlag=1;
		  ...
		  if(wanports & 0x80)
            	{
            		if(wififlag)
            			sprintf(buff_value,"%s;3",buff_value);
			else
				sprintf(buff_value,"3");
			wififlag=1;
				
        	}
		printf("\r\ncon wifiports=%s--value=%s\r\n",buff_name,buff_value);
            nvram_bufset(RT2860_NVRAM,buff_name, buff_value);
	     if(wanmode)
	     {
	     		wan_routerconfig(hl2mp_wan_config+wanvid_utag[k]);
	     	}
            /*
            if(wanmode == 1)//router                           
           {    
                ...
                {   
                    hexi_vlan[j].vlanid = r+m-1;//因为m从1开始
                    hexi_vlan[j].portmem = ((wanports & 0x0f) | (0x1 <<5));
                    hexi_vlan_v[i].vlan_vid = r+m-1;//因为m从1开始
                    hexi_vlan_v[i].portmem_v = 0;
                ...
                {
                    hexi_vlan_v[i].vlan_vid = r+m-1;//因为m从1开始
                    hexi_vlan_v[i].portmem_v |= ((wanports & 0x30) >> 4);
                    hexi_vlan_v[i].type_v=(m&0x0f);
                }
                    i++;
                //建立wan端vlan
                    hexi_vlan[j].vlanid = wanvid;
                    hexi_vlan[j].portmem = (0x1 << 4 | 0x1 <<5);
                    hexi_vlan_v[i].vlan_vid = wanvid;
                    hexi_vlan_v[i].portmem_v = 0;
                    hexi_vlan_v[i].type_v=(m&0x0f)<<4;
                    j++;
                    i++;
                    m++;
                    DBG_PRINT("j is %d, hexi_vlan[j].portmem is %d hexi_vlan_v[i].type_v %d \n", j, hexi_vlan[j-1].portmem, hexi_vlan_v[i-1].type_v);
             }
            ...
            if(wanmode == 0)//ap
           {
                ...
                {
                   hexi_vlan[j].vlanid = wanvid;
                   hexi_vlan[j].portmem = ((wanports & 0x0f) | 0x1 <<4);
                   
                }
                if(wanports & 0x30)//
                {
                   hexi_vlan[j].vlanid = wanvid; 
                   hexi_vlan[j].portmem |= (0x1<<5 | 0x1<<4);
                   hexi_vlan_v[i].vlan_vid = wanvid;
                   hexi_vlan_v[i].portmem_v = ((wanports & 0x30) >> 4);
                   i++;//
                }
                DBG_PRINT("j is %d, hexi_vlan[j].portmem is %d \n", j, hexi_vlan[j].portmem);
                j++;//
           }*/
...
/*
    for(k= 0; k < j; k++)
    {      
       if( hexi_vlan[k].vlanid != 0 )
       {
           sprintf(buff_name, "vlan%did", k+1);
           sprintf(buff_value, "%d", hexi_vlan[k].vlanid);
           nvram_set(buff_name, buff_value);
          
           sprintf(buff_name, "vlan%dports", k+1);
           DBG_PRINT("buff_name      %s\n",buff_name);


           memset(buff_value, 0, 100);
           for(l = 0; l < WAN_MAXNUM; l++)
           {    DBG_PRINT("hexi_vlan[k].portmem  %d  \n", hexi_vlan[k].portmem);
                
                if(hexi_vlan[k].vlanid ==1)
                {
                hexi_vlan[k].portmem |= (0x01 << 4 | 0x01 << 5);
                }
               if( hexi_vlan[k].portmem & (0x01 << l) )
               {
                   if(l < 4)
                   {
                   sprintf(buff_name2, "port%dpvid", l);
                   sprintf(buff_value2, "%d", hexi_vlan[k].vlanid);
                   nvram_set(buff_name2, buff_value2);
                   }
                   if(l == 4 && hexi_vlan[k].vlanid !=1)
                   {
                   sprintf(buff_value, "%s %dt", buff_value, l);
                   }
                   else
                   {
                   sprintf(buff_value, "%s %d", buff_value, l);
                   }
                 DBG_PRINT("buff_value %s \n",buff_value);
               }
           }
           }
    ...
    for(k= 0; k < WAN_MAXNUM+PORT_WNUM; k++)//将m-1到5的lanifname设置为空      
    { DBG_PRINT("lan_ifname     %d\n", k);        
    if(k==0)            
        {            
    ...
    {   
        ...
        {               
        memset(buff_value2, 0, 100);            
        for(l = 0; l < 8; l++)            
        {                    
        if( hexi_vlan_v[k].portmem_v & (0x01 << l) )                
        {                       
            if(l == 0)                    
            {                    
                sprintf(buff_value2, "eth1");                        
                if(lq_flag == 0)                        
                    {                            
                    lq = lq+l;                            
                    lq_flag=1;                        
                ...
                {                    
                sprintf(buff_value2, "%s wl0.%d", buff_value2, l);                        
                if(lq_flag == 0)                        
                    {                            
                    lq = lq+l;                            
                    lq_flag=1;                        
            ...
            DBG_PRINT("buff_value2 %s \n",buff_value2);
            sprintf(buff_name, "vlan_v%did", k+1);//vconfig  需要建立的vlan
            ...
            if(hexi_vlan_v[k].type_v == 0)
            {
                if(*buff_value2 != NULL)
                {
                     ...
                     memset(buff_value, 0, 100);
                     sprintf(buff_value, "%s vlan%d", buff_value2, hexi_vlan_v[k].vlan_vid);
                     nvram_set(buff_name, buff_value);
                }
            }
            ...
            DBG_PRINT("hexi_vlan_v[k].type_v %d\n",hexi_vlan_v[k].type_v&0x0f);
                if((hexi_vlan_v[k].type_v & 0x0f) == 1)
                    {
                    sprintf(buff_value, "br%d", hexi_vlan_v[k].type_v & 0x0f-1);
                    nvram_set("lan_ifname", buff_value);
                    memset(buff_value, 0, 100);
                    sprintf(buff_value, "%s vlan%d", buff_value2, hexi_vlan_v[k].vlan_vid);
                    nvram_set("lan_ifnames", buff_value);
                    }
                else
                    {
            ...
            {    DBG_PRINT("%d %d %d =====/n",hexi_vlan_v[k].type_v&0xf0,(hexi_vlan_v[k].type_v & 0xf0) >> 4,((hexi_vlan_v[k].type_v&0xf0) >> 4)-1);
                ...
                sprintf(buff_value, "vlan%d", hexi_vlan_v[k].vlan_vid);
                nvram_set(buff_name, buff_value);
                nvram_set("wan_ifnames",buff_value);
    ...
    //为局端管理添加vlan
     if(wanvid_utag_1 == 0)
        {
        sprintf(buff_name, "vlan%dports", j+1);
        nvram_set(buff_name, "4 5");
        sprintf(buff_name, "vlan%did", j+1);
        nvram_set(buff_name, "1");
        
        j++;
        }
        nvram_set("port4pvid", "1");

    for(k = j+1; k<17; k++)//将j+1到16的vlan设置为空
    {
        DBG_PRINT("i     %d\n", k);
        sprintf(buff_name, "vlan%dports", k);
        nvram_unset(buff_name);
        sprintf(buff_name, "vlan%did", k);
        nvram_unset(buff_name);
    }
    for(k = i+1; k<13; k++)//将i+1到12的vlan_v设置为空
    {
        DBG_PRINT("i     %d\n", k);
