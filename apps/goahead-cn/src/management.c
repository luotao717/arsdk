...
static void setWizard(webs_t wp, char_t *path, char_t *query)
{
	...
	char lan_ipaddr[16];
	char tmpBuffer1[36]={0};
	char tmpBuffer2[80]={0};
	char tmpathmode[16]={0};
	char_t *tmpstr1=NULL,*tmpstr2=NULL;
	...
	LFW(ssid, ssid);
	LFW(wireless_mode, wirelessmode);
	...
	//printf("\r\n--tt--%s",security_mode);
	strcpy(tmpBuffer1, nvram_bufget(RT2860_NVRAM, "AuthMode"));
	//printf("\r\n--tt1--%s--oo--%s",security_mode,tmpBuffer1);
	//printf("\r\n--tt2--%s",security_mode);
	if(!strcmp(security_mode,"Disable"))
		strcpy(tmpathmode,"OPEN");
	else
		strcpy(tmpathmode,security_mode);
	tmpstr2=strchr(tmpBuffer1,';');
	//printf("\r\n--tt3--%s--%s",security_mode);
	if(tmpstr2 != NULL)
	{
		//printf("\r\n--tt4--%s--iii-",security_mode);
		sprintf(tmpBuffer2,"%s%s",tmpathmode,tmpstr2);
		nvram_bufset(RT2860_NVRAM, "AuthMode", tmpBuffer2);
		//printf("\r\n--tt5--%s--yy--%s",security_mode,tmpBuffer2);
	}
	else
	{
		//printf("\r\n--tt6--%s",security_mode);
		nvram_bufset(RT2860_NVRAM, "AuthMode", tmpathmode);
		//printf("\r\n--tt7--%s",security_mode);
	}
#if 1
	...
	{ 
		printf("\r\n--tt8--%s",security_mode);
		strcpy(tmpBuffer1, nvram_bufget(RT2860_NVRAM, "EncrypType"));
		tmpstr2=strchr(tmpBuffer1,';');
		if(tmpstr2 != NULL)
		{
			sprintf(tmpBuffer2,"%s%s","WEP",tmpstr2);
			nvram_bufset(RT2860_NVRAM, "EncrypType", tmpBuffer2);
		}
		else
		{
		...
		strcpy(tmpBuffer1, nvram_bufget(RT2860_NVRAM, "DefaultKeyID"));
		
		tmpstr2=strchr(tmpBuffer1,';');
		if(tmpstr2 != NULL)
		{
			sprintf(tmpBuffer2,"%s%s","1",tmpstr2);
			nvram_bufset(RT2860_NVRAM, "DefaultKeyID", tmpBuffer2);
		}
		else
		{
		...
		{
			// HEX
			strcpy(tmpBuffer1, nvram_bufget(RT2860_NVRAM, "Key1Type"));
			tmpstr2=strchr(tmpBuffer1,';');
			if(tmpstr2 != NULL)
			{
				sprintf(tmpBuffer2,"%s%s","0",tmpstr2);
				nvram_bufset(RT2860_NVRAM, "Key1Type", tmpBuffer2);
			}
			else
			{
				nvram_bufset(RT2860_NVRAM, "Key1Type", "0");
			}
		}
		else if (5 == strlen(security_key) || 13 == strlen(security_key))
		{
			...
			{
				nvram_bufset(RT2860_NVRAM, "Key1Type", "1");
			}
		}

		strcpy(tmpBuffer1, nvram_bufget(RT2860_NVRAM, "Key1Str1"));
		tmpstr2=strchr(tmpBuffer1,';');
		if(tmpstr2 != NULL)
		{
			sprintf(tmpBuffer2,"%s%s",security_key,tmpstr2);
			nvram_bufset(RT2860_NVRAM, "Key1Str1", tmpBuffer2);
		}
		else
		{
	...
	else if (!strcmp(security_mode, "WPAPSK") || !strcmp(security_mode, "WPA2PSK") || !strcmp(security_mode, "WPAPSKWPA2PSK"))
	{
		...
		if(tmpstr2 != NULL)
		{
			sprintf(tmpBuffer2,"%s%s","AES",tmpstr2);
			nvram_bufset(RT2860_NVRAM, "EncrypType", tmpBuffer2);
		}
		else
		{
		...
		strcpy(tmpBuffer1, nvram_bufget(RT2860_NVRAM, "RekeyInterval"));
		tmpstr2=strchr(tmpBuffer1,';');
		if(tmpstr2 != NULL)
		{
			sprintf(tmpBuffer2,"%s%s","3600",tmpstr2);
			nvram_bufset(RT2860_NVRAM, "RekeyInterval", tmpBuffer2);
		}
		else
		{
			nvram_bufset(RT2860_NVRAM, "RekeyInterval", "3600");
		}
		strcpy(tmpBuffer1, nvram_bufget(RT2860_NVRAM, "RekeyMethod"));
		tmpstr2=strchr(tmpBuffer1,';');
		if(tmpstr2 != NULL)
		{
			sprintf(tmpBuffer2,"%s%s","TIME",tmpstr2);
			nvram_bufset(RT2860_NVRAM, "RekeyMethod", tmpBuffer2);
		}
		else
		{
	...
	else
	{
		...
		if(tmpstr2 != NULL)
		{
			sprintf(tmpBuffer2,"%s%s","NONE",tmpstr2);
			nvram_bufset(RT2860_NVRAM, "EncrypType", tmpBuffer2);
			printf("\r\n--tt--%s",security_mode);
		}
		else
...
{
	char_t buf[32];
	long long data = getIfStatistic( "ath0", RXPACKET);
	snprintf(buf, 32, "%lld", data);	
	websWrite(wp, T("%s"), buf);
	return 0;
}
...
{
	char_t buf[32];
	long long data = getIfStatistic( "ath0", TXPACKET);
	snprintf(buf, 32, "%lld", data);	
	websWrite(wp, T("%s"), buf);
	return 0;
}
...
{	
	 ...
	 */
		...
		websWrite(wp, T("<meta http-equiv=\"content-type\" content=\"text/html;charset=utf-8\">\n"));\
		websWrite(wp, T("<script language=\"javascript\" src=\"../js/language_%s.js\"></script>\n"),langType);\
		websWrite(wp, T("<SCRIPT language=javascript type=text/javascript>var rebootInf = new Array(\"%s\",80,58,1,0,0 );</SCRIPT><LINK href=\"/dynaform/css_main.css\" type=text/css rel=stylesheet><SCRIPT language=javascript src=\"/dynaform/common.js\"  type=text/javascript></SCRIPT><script language=JavaScript><!--\n"),lan_ip);\
		websWrite(wp, T("function LoadMain(){parent.location.href = 'http://' + rebootInf[0] + ':' + rebootInf[1];}function doLoad(){doReFresh();}var timeout = rebootInf[2] * 10;var rate = 0;function doReFresh(){window.setTimeout(\"growUp()\",timeout);}function growUp(){rate = rate + 1;document.getElementById(\"td1\").style.width = rate + \"%%\";var per=document.getElementById(\"percent\");per.innerHTML=rate+\"%%\";if(rate >= 100){var obj=document.getElementById(\"t_notice\"); obj.style.display=\"block\";document.getElementById(\"t_restart\").style.display=\"none\";document.getElementById(\"t_complete\").style.display=\"block\";document.getElementById(\"t_complete\").style.color=\"red\";per.style.color=\"red\";LoadMain();return;}doReFresh();}\n"));\
		websWrite(wp, T("//-->\n"));\
		websWrite(wp,T("</script>"));\
		websWrite(wp, T("</head>\n\n"));\
		websWrite(wp, T("<BODY onload=\"return doLoad()\">"));\
		websWrite(wp, T("<CENTER><FORM><TABLE border=0><TBODY> <TR><TD class=h1 id=t_title>"));\
		websWrite(wp, T("<script>dw(MM_load_factory_default)</script></TD></TR><TR><TD class=blue></TD></TR>"));\
		websWrite(wp, T("<TR><TD class=h2 id=t_restart><script>dw(HC_reboot_now)</script></TD></TR>"));\
		websWrite(wp, T("<TR><TD class=h2 id = \"t_upgrade\"><script>dw(MM_load_factory)</script></TD></TR>"));\
		websWrite(wp, T("<TR><TD class=h2 id=t_complete style=\"DISPLAY: none\"><script>dw(HC_completed)</script></TD></TR>"));\ 
		websWrite(wp, T("<TR><TD style=\"PADDING-LEFT: 5%%\" align=middle><TABLE class=space height=10 cellSpacing=0 cellPadding=0 width=\"40%%\" align=center border=0><TBODY><TR><TD><DIV id=percent></DIV></TD><TD style=\"WIDTH: 98\%%\"><TABLE class=guage id=td1 style=\"WIDTH: 0px\" height=10 cellSpacing=0 cellPadding=0 border=0><TBODY><TR><TD></TD></TR></TBODY></TABLE></TD></TR></TBODY></TABLE></TD></TR><TR> <TD class=h2 id=t_notice style=\"DISPLAY: none\"><script>dw(HC_completed_info)</script></TD></TR><TR><TD class=blue></TD></TR></TBODY></TABLE></FORM></CENTER></BODY></html>\n"));\
		websDone(wp, 200);\
		exit(1);
	  }
	  
//	system("apdown");
	//system("rmmod athrs_gmac");
...
static void RebootSystem(webs_t wp, char_t *path, char_t *query)
{
	 ...
	 */
		...
		websWrite(wp, T("<BODY onload=\"return doLoad()\">"));\
		websWrite(wp, T("<CENTER><FORM><TABLE border=0><TBODY> <TR><TD class=h1 id=t_title>"));\
		websWrite(wp, T("<script>dw(BT_reboot)</script></TD></TR>"));\
		websWrite(wp, T("<TR><TD class=blue></TD></TR>"));\
		websWrite(wp, T("<TR><TD class=h2 id=t_restart><script>dw(HC_reboot_now)</script></TD></TR>"));\
		websWrite(wp, T("<TR><TD class=h2 id = \"t_upgrade\"><script>dw(MM_reboot_system)</script></TD></TR>"));\
		websWrite(wp, T("<TR><TD class=h2 id=t_complete style=\"DISPLAY: none\"><script>dw(HC_completed)</script></TD></TR>"));\ 
		websWrite(wp, T("<TR><TD style=\"PADDING-LEFT: 5%%\" align=middle><TABLE class=space height=10 cellSpacing=0 cellPadding=0 width=\"40%%\" align=center border=0><TBODY><TR><TD><DIV id=percent></DIV></TD><TD style=\"WIDTH: 98\%%\"><TABLE class=guage id=td1 style=\"WIDTH: 0px\" height=10 cellSpacing=0 cellPadding=0 border=0><TBODY><TR><TD></TD></TR></TBODY></TABLE></TD></TR></TBODY></TABLE></TD></TR><TR> <TD class=h2 id=t_notice style=\"DISPLAY: none\"><script>dw(HC_completed_info)</script></TD></TR><TR><TD class=blue></TD></TR></TBODY></TABLE></FORM></CENTER></BODY></HTML>\n"));
		websDone(wp, 200);\
		exit(1);
