...
static void iptablesAppFilterRun(void)
{
	int i = 0;
	char entry[256];
	char *limitMac = (char *)nvram_bufget(RT2860_NVRAM, "LimitMac");
	
	while( (getNthValueSafe(i++, limitMac, ';', entry, sizeof(entry)) != -1) ){
		if(strlen(entry))
			doSystem("iptables -I FORWARD -m mac --mac-source %s -j DROP", entry);
	}
...
static int showIPPortFilterRulesASP(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	switch(atoi(default_policy)){
		case 0:
			websWrite(wp, T("<tr><td align=center colspan=6 id=portCurrentFilterDefaultAccept><script>dw(JS_msg47)</script></td><td align=center id=pktCnt%d>-</td></tr>"), i);
			break;
		case 1:
			websWrite(wp, T("<tr><td align=center colspan=6 id=portCurrentFilterDefaultDrop><script>dw(JS_msg47)</script></td><td align=center id=pktCnt%d>-</td></tr>"), i);
			break;
	}
...
static void singlePortForwardBasicSettings(webs_t wp, char_t *path, char_t *query)
{
	char_t  *submitUrl = websGetVar(wp, T("submit-url"), T(""));

	char *single_portforwad_enable = websGetVar(wp, T("singleforwardEnabled"), T(""));//add by chenfei

	if(!single_portforwad_enable && !strlen(single_portforwad_enable))
		return;

	if(atoi(single_portforwad_enable)) {	// user choose nothing but press "apply" only
...
static void singlePortForward(webs_t wp, char_t *path, char_t *query)
{
	...
	char rule[8192];
	char *ip_address, *publicPort, *privatePort, *protocol, *comment;
...
static void ipportBasicSettings(webs_t wp, char_t *path, char_t *query)
{
	char_t  *submitUrl = websGetVar(wp, T("submit-url"), T(""));
...
static void websSysRemote(webs_t wp, char_t *path, char_t *query)
{
	...
	nvram_bufset(RT2860_NVRAM, "RemoteManagement", rmE);
	nvram_commit(RT2860_NVRAM);
...
void formDefineFirewall(void)
{
	...
	websFormDefine(T("ipportBasicSettings"), ipportBasicSettings);
	...
	websFormDefine(T("singlePortForwardBasicSettings"), singlePortForwardBasicSettings);//add by chenfei for single port forward
