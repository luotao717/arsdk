...
static int  getIndex(int eid, webs_t wp, int argc, char_t **argv);
static int  getTitle(int eid, webs_t wp, int argc, char_t **argv);
static int  getFavicon(int eid, webs_t wp, int argc, char_t **argv);
static int  getStyle(int eid, webs_t wp, int argc, char_t **argv);
static int  getCharset(int eid, webs_t wp, int argc, char_t **argv);
static int  getCaryBuilt(int eid, webs_t wp, int argc, char_t **argv);
static int  getMacPtBuilt(int eid, webs_t wp, int argc, char_t **argv);
static int  getPppoeSpecBuilt(int eid, webs_t wp, int argc, char_t **argv);
static int  getProductModel(int eid, webs_t wp, int argc, char_t **argv);
static int  getCustomerUrl(int eid, webs_t wp, int argc, char_t **argv);
static int  getCustomerUrlBuilt(int eid, webs_t wp, int argc, char_t **argv);
...
void formDefineUtilities(void)
{
	...
	websAspDefine(T("getTitle"), getTitle);
	websAspDefine(T("getFavicon"), getFavicon);
	websAspDefine(T("getStyle"), getStyle);
	websAspDefine(T("getCharset"), getCharset);
	websAspDefine(T("getCaryBuilt"), getCaryBuilt);
	websAspDefine(T("getMacPtBuilt"), getMacPtBuilt);
	websAspDefine(T("getPppoeSpecBuilt"), getPppoeSpecBuilt);
	websAspDefine(T("getProductModel"), getProductModel); 
	websAspDefine(T("getCustomerUrl"), getCustomerUrl);
	websAspDefine(T("getCustomerUrlBuilt"), getCustomerUrlBuilt);
...
static int getIndex(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	if ( !strncmp(field, T("ddnsStatus"),11) ) {		
		...
		int ddns_status, wan_ip_flag=0;			

		if (atoi(ddns_enabled)==0){			
			...
			if (atoi(opmode)==1) {
				...
				if (p==NULL) {	//Is connected					
					if (!strcmp(wanmode, "PPPOE")) {	
						wan_ip_flag = getIfIp("ppp0", wan_ip);
					}
					else if (!strcmp(wanmode, "DHCP") || !strcmp(wanmode, "STATIC")) {
						wan_ip_flag = getIfIp("br1", wan_ip);
					}
						
					if (wan_ip_flag!=-1) {//has wan ip
	...
	else if ( !strncmp(field, T("Dlna"),4) ) 	
				return websWrite(wp,T("%s"),"1");
	#endif
	else if(!strncmp(field, T("getUserGroup"),12) )
	{
		...
		ejSetResult(eid,tmpBuf);
		return 0;
	}
	else if ( !strncmp(field, T("usbStatus"),10) ) {		
		int i, j,usb_status;
		char usbstatusBUf[32]={0};
		for (i=97; i<123; i++) {
			sprintf(tmpBuf,"/media/sd%c",i);
			usb_status = stat(tmpBuf, &st);
			if (!usb_status) 
				break;
			else
			{
				for(j=1;j<10;j++)
				{
					sprintf(usbstatusBUf,"%s%d",tmpBuf,j);
					printf("\r\ntest --%s",usbstatusBUf);
					usb_status = stat(usbstatusBUf, &st);
					if (!usb_status) 
						break;
				}
				if (!usb_status) 
	...
	else if ( !strncmp(field, T("apcliStatus"),12) ) {
		char *apclimode = (char *)nvram_bufget(RT2860_NVRAM, "ApCliEnable");
		if (atoi(apclimode)==1) {			
	...
	else if ( !strncmp(field, T("wanLinkStatus"),14) ) {
		char *opmode = (char *)nvram_bufget(RT2860_NVRAM, "OperationMode");
		if (atoi(opmode)==1) {
		#if defined CONFIG_WAN_AT_P4
			system("mii_mgr -g -p 4 -r 1 > /tmp/wanConnectStatus");
	...
	else if ( !strncmp(field, T("wanConnectStatus"),17) ) {
		char *opmode = (char *)nvram_bufget(RT2860_NVRAM, "OperationMode");
		char *wanmode = (char *)nvram_bufget(RT2860_NVRAM, "wanConnectionMode");
		if (atoi(opmode)==1) {
		#if defined CONFIG_WAN_AT_P4
		...
		else if (atoi(opmode)==3) {	//First check the state of apclient
			if (!strcmp(wanmode, "DHCP")) {				
...
static int getLangBuilt(int eid, webs_t wp, int argc, char_t **argv)
{
#if defined (CONFIG_USER_LANG_ALL)
	return websWrite(wp, T("1"));
#else
...
static int getTitle(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	return websWrite(wp, T("GKC204NUW"));
#endif
}
...
{
#if defined (CONFIG_LKTOS_PRIVATE_OEM_NAME_TOTOLINK)
	return websWrite(wp, T("totolink.ico"));
#else
	return websWrite(wp, T("favicon.ico"));
#endif
}
...
{
#if defined (CONFIG_LKTOS_PRIVATE_OEM_NAME_TOTOLINK)
	return websWrite(wp, T("totolink/style.css"));
#elif defined (CONFIG_LKTOS_PRIVATE_OEM_NAME_TONGYONG)
	return websWrite(wp, T("standard/style.css"));
#endif
}

static int getCharset(int eid, webs_t wp, int argc, char_t **argv)
{
	return websWrite(wp, T("utf-8"));
}

static int getCaryBuilt(int eid, webs_t wp, int argc, char_t **argv)
{
#if defined (CONFIG_LKTOS_PRIVATE_OEM_NAME_CARY)
	return websWrite(wp,T("%s"),"0");
#else
	return websWrite(wp,T("%s"),"0");
#endif
...
static int getPppoeSpecBuilt(int eid, webs_t wp, int argc, char_t **argv)
{
#if defined (CONFIG_USER_LANG_ALL) || defined (CONFIG_USER_CN)
	return websWrite(wp,T("%s"),"1");
#else
	return websWrite(wp,T("%s"),"0");
#endif	
...
static int getProductModel(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	return websWrite(wp,T("%s"),"Model No.: WR-NET-022-ZI");
#else
	return websWrite(wp,T("%s"),"&nbsp;");
#endif	
}
...
{
#if defined (CONFIG_LKTOS_PRIVATE_OEM_NAME_CDRKING)
	return websWrite(wp,T("%s"),"www.cdrking.com");
#elif defined (CONFIG_LKTOS_PRIVATE_OEM_NAME_NBOX)
	return websWrite(wp,T("%s"),"www.intelbras.com.br");
#else
	return websWrite(wp,T("%s"),"&nbsp;");
#endif	
}
...
static int getCustomerUrlBuilt(int eid, webs_t wp, int argc, char_t **argv)
{
#if defined (CONFIG_USER_CUSTOMER_URL)
	return websWrite(wp,T("%s"),"1");
#else
	return websWrite(wp,T("%s"),"0");
#endif	
...
static void setOpMode(webs_t wp, char_t *path, char_t *query)
{
	...
	else if (!strncmp(old_mode, "1", 2) || !strncmp(old_mode, "3", 2)) {//gateway | wisp
		if (!strncmp(mode, "0", 2)) {
#if defined CONFIG_RAETH_ROUTER || defined CONFIG_MAC_TO_MAC_MODE || defined CONFIG_RT_3052_ESW || defined CONFIG_ICPLUS_PHY
#else
			/*
