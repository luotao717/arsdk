...
#define UPLOAD_FIRMWARE_SUPPORT 1
...
inline int mtd_write_firmware(char *filename, int offset, int len,int type)
{
	char cmd[512];
	int status;
  ...
  if( 1 == type)
  {
  	snprintf(cmd, sizeof(cmd), "/sbin/mtd_write -o %d -l %d write %s uImage", offset, len, filename);
   	status = system(cmd);
	//printf("21212");
  }
  else if( 2 == type)
  {
  	snprintf(cmd, sizeof(cmd), "/sbin/mtd_write -o %d -l %d write %s rootfs", offset, len, filename);
   	status = system(cmd);
	//printf("tttttt");
  }
  else
