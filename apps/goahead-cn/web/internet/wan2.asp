...
function vlanSetSwitch()
{
	//alert("fdsfd");
	if (document.wanCfg.vlanSetEnbl.options.selectedIndex == 1)
	{
		document.getElementById("vlantagRow").style.display = "";
		document.getElementById("vlanprioRow").style.display = "";
	}
	else
	{
		//alert("ddd");
		document.getElementById("vlantagRow").style.display = "none";
		document.getElementById("vlanprioRow").style.display = "none";
	}
}

function workconnectionTypeSwitch()
{
	document.getElementById("wanroutemodesel").style.display = "none";
	...
	if (document.wanCfg.workconnectmode.options.selectedIndex == 0)
	{
		document.getElementById("wanroutemodesel").style.display = "";
...
function Load_Setting()
{
	...
	var connect1mode = <% getCfgZero(1, "Connect2OperationMode"); %>;
	var connect1ethports = "<% getCfgGeneral(1, "Connect2ethports"); %>";
	var connect2ethports = "<% getCfgGeneral(1, "Connect1ethports"); %>";
	var connect1wifiports = "<% getCfgGeneral(1, "Connect2wifiports"); %>";
	var connect2wifiports = "<% getCfgGeneral(1, "Connect1wifiports"); %>";
	var connect1vlanen = "<% getCfgGeneral(1, "Connect2vlanEn"); %>";
//	var connect1vlanid = "<% getCfgGeneral(1, "Connect2vlanid"); %>";
	var pptpb = <% getPPTPBuilt(); %>;
	var l2tpb = <% getL2TPBuilt(); %>;
	var dongle3gb = <% get3GBuilt(); %>;
	var pppoe_xkjs = <% getPppoeSpecBuilt(); %>;
	var connect1wifiportsarr= connect1wifiports.split(";");
	var wifilen = connect1wifiportsarr.length;
	var connect2wifiportsarr= connect2wifiports.split(";");
	var wifilen2 = connect2wifiportsarr.length;
	if (opmode == "3") {
		document.getElementById("div_macclone_br").style.display = "none";
		document.getElementById("div_macclone").style.display = "none";
	}	
	...
	if(connect1ethports == "3")
		document.wanCfg.ethport3.checked="true";
	if(connect2ethports == "3")
		document.wanCfg.ethport3.disabled="true";
	for(var i=0;i<wifilen;i++)
	{
		if(connect1wifiportsarr[i] == "")
			continue;
		var tmpstr="wifiport"+connect1wifiportsarr[i];
		var tmphandle=document.getElementById(tmpstr);
		tmphandle.checked="true";
	}
	for(var i=0;i<wifilen2;i++)
	{
		if(connect2wifiportsarr[i] == "")
			continue;
		var tmpstr="wifiport"+connect2wifiportsarr[i];
		var tmphandle=document.getElementById(tmpstr);
		tmphandle.disabled="true";
	}
	if(connect1vlanen == "1")
		document.wanCfg.vlanSetEnbl.options.selectedIndex = 1;
	else
		document.wanCfg.vlanSetEnbl.options.selectedIndex = 0;
	if(connect1mode == "0")
	{
		document.wanCfg.workconnectmode.options.selectedIndex = 0;
		workconnectionTypeSwitch();
	}
	else
	{
		document.wanCfg.workconnectmode.options.selectedIndex = 1;
		workconnectionTypeSwitch();
	}
	if (clone_mode == 1) {
...
<tr><td class="title"><script>dw(VLAN_bridge_settings_title)</script></td></tr>
<tr><td><script>dw(VLAN_bridge_settings_com)</script></td></tr>
<tr><td><hr><% checkSuperUser(); %></td></tr>
</table>

<form method=post name="wanCfg" action="/goform/setWan2">
<input type="hidden" name="submit-url" value="/internet/wan2.asp">
<input type="hidden" name="syncWithPcTime" value="081510102012">
<table width=100% border=0 cellpadding=3 cellspacing=1> 
<tr>
  <td class="thead"><script>dw(VLAN_connectname)</script> :</td>
  <td>connect2</td>
</tr>
<tr>
  <td class="thead"><script>dw(VLAN_mode)</script>:</td>
  <td><select name="workconnectmode" onChange="workconnectionTypeSwitch();" disabled="disabled">
      <option value="0"><script>dw(VLAN_route)</script></option>
      <option value="1"><script>dw(VLAN_bridge)</script></option>
    </select></td>
</tr>
<tr id="wanroutemodesel">
...
<br id="div_vlan_br" style="display:none">
<table id="vlantotal" width=100% border=0 cellpadding=3 cellspacing=1> 
<tr>
  <td class="title2" colspan="2"><script>dw(VLAN_settings_title)</script><hr></td>
</tr>
<tr id="portband">
...
<tr id="vlantagset">
  ...
  <td><select name="vlanSetEnbl" onChange="vlanSetSwitch()">
      <option value="0"><script>dw(MM_disable)</script></option>
      <option value="1"><script>dw(MM_enable)</script></option>
    </select></td>
</tr>
<tr id="vlantagRow">
  <td class="thead">vlan id:</td>
  <td><input name="vlan1id" id="vlan1id" maxlength=17 value="<% getCfgGeneral(1, "Connect2vlanid"); %>"> 
  </td>
</tr>
<tr id="vlanprioRow">
  <td class="thead">vlan Priority:</td>
  <td><input name="vlan1prio" id="vlan1prio" maxlength=17 value="<% getCfgGeneral(1, "Connect2vlanprio"); %>"> 
  </td>
</tr>
</table>
...
<table width=100% border=0 cellpadding=3 cellspacing=1> 
  <tr>
    <td>
      ...
      <input type=button class=button value="'+BT_reset+'" onClick="resetForm();">')</script><% needRebootWaring(); %>
    </td>
  </tr>
</table>
</form>
