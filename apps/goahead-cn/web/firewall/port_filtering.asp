...
function updateState()
{
    if (! rules_num ){
		...
		//alert(document.ipportFilterDelete.deleteSelFilterPort.value);
...
<table width=100% border=0 cellpadding=3 cellspacing=1> 
  <tr>
    ...
    <td>
      <script>dw('<input type=submit class=button value="'+BT_delete+'" name="deleteSelFilterPort" id="deleteSelFilterPort" onClick="return deleteClick()"> &nbsp; &nbsp;\
      <input type=button class=button value="'+BT_reset+'" name="reset" onClick="resetForm();">')</script>
    </td>
  </tr>
</table>
