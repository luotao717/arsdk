...
function formCheck()
{
	...
	{
		//name
		if (!blankCheck(document.storage_ftp.ftp_name, MM_ftp_name)) 
			return false;
		
		//port
		if (!blankCheck(document.storage_ftp.ftp_port, MM_ftp_port)) 
			return false;
			
		if (!portCheckMsg(document.storage_ftp.ftp_port, MM_ftp_port)) 
			return false;
		
		//sessions
		if (!numCheckNullMsg(document.storage_ftp.ftp_max_sessions, MM_ftp_max_sessions)) 
			return false;
	}
...
<tr style="display:none"><td><script>dw(JS_msg162)</script></td></tr>
<tr><td><hr></td></tr>
</table>
...
<table width=100% border=0 cellpadding=3 cellspacing=1> 
  ...
  <tr>
    ...
    <td><input type=text name=ftp_name maxlength=16 value="Intelbras"></td>
  </tr>
  <tr> 
