...
function formCheck()
{
	switch(document.disk_parted.max_part.value)
	{
		case "4":
		case "3":
			if (!rangeCheckNullMsg(document.disk_parted.part3_vol, 1, 100000, MM_partition+" 3")) 
				return false;
			
			maxvol -= document.disk_parted.part3_vol.value;
		case "2":
			if (!rangeCheckNullMsg(document.disk_parted.part2_vol, 1, 100000, MM_partition+" 2")) 
				return false;
				
			maxvol -= document.disk_parted.part2_vol.value;
		default:
			if (!rangeCheckNullMsg(document.disk_parted.part1_vol, 1, 100000, MM_partition+" 1")) 
				return false;
...
<tr style="display:none"><td><script>dw(JS_msg165)</script></td></tr>
<tr><td><hr></td></tr>
</table>
