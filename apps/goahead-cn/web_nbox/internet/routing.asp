...
function formCheck()
{
	if (!ipCheck(document.addrouting.dest, MM_destip, 0)) 
		return false; 
    
	if (document.addrouting.hostnet.selectedIndex == 1) 
	{ 
		if (!maskCheck(document.addrouting.netmask, MM_submask)) 
			return false; 
	}
		
	if (!ipCheck(document.addrouting.gateway, MM_gateway_address, 0)) 
		return false; 
	
	/*
	if (document.addrouting.interface.value == "Custom") 
	{ 
		if (!blankCheck(document.addrouting.custom_interface, MM_interface)) 
			return false; 
	}
	*/
...
<tr style="display:none"><td><script>dw(JS_msg149)</script></td></tr>
<tr><td><hr></td></tr>
</table>
...
<tr style="display:none">
	...
	<td><select name="hostnet" onChange="hostnetChange()">
		...
		<option value="net"><script>dw(MM_net)</script></option>
		</select></td>
</tr>
...
<tr>
	...
	<td><select name="interface" onChange="interfaceChange()">
		...
		<!--<option value="Custom"><script>dw(MM_custom)</script></option>-->
		</select>
...
<table width=100% border=1 cellpadding=3 style="border-collapse: collapse" bordercolor="#C8C8C8">
	<tr style="display:none">
		<td class="title2" colspan="9"><script>dw(MM_routing_table_list)</script>:</td>
	</tr>
	<tr bgcolor=#f2f2f2>
