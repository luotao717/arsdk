...
function formCheck()
{
	if (document.ipv6_cfg.ipv6_opmode.value == "1") {
		...
		if (document.ipv6_cfg.ipv6_lan_ipaddr.value != "") {
			if (!ipv6AddrCheck(document.ipv6_cfg.ipv6_lan_ipaddr, 128, "LAN IPv6 IP address"))  
				return false;
			
			if (!rangeCheck(document.ipv6_cfg.ipv6_lan_prefix_len, 0, 128, "LAN Prefix length"))  
				return false;
		}
		
		if (document.ipv6_cfg.ipv6_wan_ipaddr.value != "") {
			if (!ipv6AddrCheck(document.ipv6_cfg.ipv6_wan_ipaddr, 128, "WAN IPv6 IP address"))  
				return false;
			
			if (!rangeCheck(document.ipv6_cfg.ipv6_wan_prefix_len, 0, 128, "WAN Prefix length"))  
				return false;
		}
		
		if (document.ipv6_cfg.ipv6_static_gw.value != "") { 
			if (!ipv6AddrCheck(document.ipv6_cfg.ipv6_static_gw, 128, "Default Gateway"))  
				return false; 
		}
	} 
		...
		if (!ipv6AddrCheck(document.ipv6_cfg.ipv6_6rd_prefix, 32, "ISP 6rd Prefix address"))  
			return false;
		
		if (!rangeCheck(document.ipv6_cfg.ipv6_6rd_prefix_len, 0, 32, "ISP 6rd Prefix length"))  
			return false;
		
		if (!ipCheck(document.ipv6_cfg.ipv6_6rd_border_ipaddr, "ISP Border Relay IPv4 address", 0))  
			return false;
	} 
		...
		if (!ipv6AddrCheck(document.ipv6_cfg.ipv6_ds_wan_ipaddr, 128, "WAN IPv6 IP address"))  
			return false;
		
		if (!ipv6AddrCheck(document.ipv6_cfg.ipv6_ds_aftr_ipaddr, 128, "AFTR Server IPv6 IP address"))  
			return false;
		
		if (!ipv6AddrCheck(document.ipv6_cfg.ipv6_ds_gw_ipaddr, 128, "Gateway IPv6 IP address"))  
			return false;
	}
