...
function formCheck()
{
	...
	{
		if (!blankCheck(document.DDNS.Account, MM_username)) 
			return false;
		
		if (!blankCheck(document.DDNS.Password, MM_password)) 
			return false;
		
		if (!blankCheck(document.DDNS.DDNS, MM_domainname)) 
			return false;
	}
...
{
	var ddns_enable = "<% getCfgGeneral(1, "DDNSEnabled"); %>";
	var ddns_provider = "<% getCfgGeneral(1, "DDNSProvider"); %>";
	
	document.DDNS.DDNSEnabled.options.selectedIndex = 1*ddns_enable;
	updateState();
...
<tr style="display:none"><td><script>dw(JS_msg132)</script></td></tr>
<tr><td><hr></td></tr>
</table>
...
<tr>
  ...
  <td><select onChange="showProvider()" name="DDNSProvider">
