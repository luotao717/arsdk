...
function formCheck()
{
	...
	{
		if (!portCheckMsg(document.ipportFilter.dFromPort, MM_destination+MM_1st_port)) 
			return false;
		
		if (document.ipportFilter.dToPort.value != "")
		{
			if (!portCheckMsg(document.ipportFilter.dToPort, MM_destination+MM_2nd_port)) 
				return false;
	...
	{ 
		if (!macCheck(document.ipportFilter.mac_address, MM_macaddr))  
			return false; 
	}
	...
	if (document.ipportFilter.sip_address.value != "")
	{
		if (!ipCheck(document.ipportFilter.sip_address, MM_sourceip, 0)) 
			return false;
...
<tr style="display:none"><td><script>dw(JS_msg140)</script></td></tr>
<tr><td><hr></td></tr>
</table>
...
<tr style="display:none">
	<td colspan="2"><script>dw(JS_msg141)</script></td>
</tr>
</table>
</form>
...
<input type="hidden" name="submit-url" value="/firewall/port_filtering.asp">
<p><b><script>dw(MM_ipportf_list)</script>:<script>document.write(JS_msg94);</script></b></p>
