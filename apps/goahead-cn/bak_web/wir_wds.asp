...
function CheckValue()
{
	...
	if (document.wireless_basic.wds_mode.options.selectedIndex >= 1)
	{
		if (document.wireless_basic.wds_encryp_type.options.selectedIndex >= 1)
		{
			var keyvaluewds1 = document.wireless_basic.wds_encryp_key.value;

			if (keyvaluewds1.length == 0){
				alert('请输入 aes(tkip) 密钥!');
				return false;
			}

			if (keyvaluewds1.length < 8){
				alert('请至少输入8个 AES(TKIP) 字符!');
				return false;
			}
		}
	}
	if (document.wireless_basic.wds_mode.options.selectedIndex >= 2)
	{
		...
		for (i = 1; i <= 4; i++)
		{
			...
			if (!re.test(eval("document.wireless_basic.wds_"+i).value)) {
				alert("请输入正确的远程WDS AP MAC地址! (XX:XX:XX:XX:XX:XX)");
				return false;
			}
			else {
		...
		if (document.wireless_basic.wds_encryp_type.options.selectedIndex >= 1)
		{
			var keyvaluewds = document.wireless_basic.wds_encryp_key.value;

			if (keyvaluewds.length == 0){
				alert('请输入 aes(tkip) 密钥!');
				return false;
			}

			if (keyvaluewds.length < 8){
				alert('请至少输入8个 AES(TKIP) 字符!');
				return false;
			}
		}
...
<table width="90%" border="0" cellpadding="0" cellspacing="0" align="center">
    ...
    <td style="padding-top:30px;">
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
        	<tr>
            	<td class="nav">当前路径:KN-WR922 &gt;&gt;无线设置 &gt;&gt;无线分布系统(WDS)</td>
          	</tr>
			<tr>
				<td>&nbsp;</td>
			</tr>
			<tr>
          <td class="space" style="padding-left:10px; padding-right:10px;">&nbsp;&nbsp;&nbsp;本页面提供了对WDS(无线分布系统)参数的设置,详细信息请点<input type="button" class="button5" value="帮助" onclick=popHelp('help.htm#wlwds')></td>
       </tr>
	   		  <tr>
        	<td>&nbsp;</td>
      	  </tr>
            	...
            	<td>
			  		<table class="space" width="100%" border="0" cellspacing="0" cellpadding="0">
						...
						<tr id="status0">
			  			...
			  			<tr id="status1">
                  			<td class="contentpadding">无线状态:</td>
            				<td id="status1">关闭</td>
                		</tr>
			  			<tr>
                  			<td class="contentpadding" id="basicRadioButton">无线开关:</td>
                  			<td><input type="button" class="button" name="radioButton" value="RADIO ON" onClick="if (this.value.indexOf('关闭') >= 0) RadioStatusChange(1); else RadioStatusChange(0); document.wireless_basic.submit();" /><input type=hidden name=radiohiddenButton value="2"></td>
						</tr>
                		<tr>
                  			<td class="contentpadding" id="basicNetMode">网络模式:</td>
						...
						<tr>
						  <td class="contentpadding">SSID 1:</td>
						  <td><input type=text name=mssid_1 size=20 maxlength=32 value="<% getCfgGeneral(1, "SSID2"); %>"></td>
						</tr>
						<tr>
						  <td class="contentpadding">SSID 2:</td>
						  <td><input type=text name=mssid_2 size=20 maxlength=32 value="<% getCfgGeneral(1, "SSID3"); %>"></td>
						</tr>
						<tr>
						  <td class="contentpadding">SSID 3:</td>
						  <td><input type=text name=mssid_3 size=20 maxlength=32 value="<% getCfgGeneral(1, "SSID4"); %>"></td>
						</tr>
						<tr>
						  <td class="contentpadding">SSID 4:</td>
						  <td><input type=text name=mssid_4 size=20 maxlength=32 value="<% getCfgGeneral(1, "SSID5"); %>"></td>
						</tr>
						<tr>
						  <td class="contentpadding">SSID 5:</td>
						  <td><input type=text name=mssid_5 size=20 maxlength=32 value="<% getCfgGeneral(1, "SSID6"); %>"></td>
						</tr>
						<tr>
						  <td class="contentpadding">SSID 6:</td>
						  <td><input type=text name=mssid_6 size=20 maxlength=32 value="<% getCfgGeneral(1, "SSID7"); %>"></td>
						</tr>
						<tr>
						  <td class="contentpadding">SSID 7:</td>
						  <td><input type=text name=mssid_7 size=20 maxlength=32 value="<% getCfgGeneral(1, "SSID8"); %>"></td>
						</tr>
						<tr>
						  <td class="contentpadding" id="basicBroadcastSSID">广播网络名称(SSID):</td>
						  <td><input type=radio name=broadcastssid value="1" checked>启用&nbsp;<input type=radio name=broadcastssid value="0">禁用</td>
						</tr>
						<tr>
						  <td class="contentpadding" id="basicBSSID">BSSID:</td>
            	...
            	<td>
			  		<table class="space" width="100%" border="0" cellspacing="0" cellpadding="0">
						...
						<tr id="div_wds_encryp_key" name="div_wds_encryp_key" style="visibility:visible; display:none">
						  ...
						  <td><input type=text name=wds_encryp_key size=28 maxlength=64 value="">
						  (如加密方式不为None，必须填入至少8个字符)</td>
						</tr>
						<tr id="wds_mac_list_1" name="wds_mac_list_1" style="visibility:visible; display:none">
						  ...
						  <td><input type=text name=wds_1 size=20 maxlength=17 value="">格式如 <font color="cc0000">00:ed:23:da:eb:18 </font></td>
						</tr>
						<tr id="wds_mac_list_2" name="wds_mac_list_2" style="visibility:visible; display:none">
						  <td class="contentpadding" id="basicWDSAPMacAddr">AP MAC 地址:</td>
						  <td><input type=text name=wds_2 size=20 maxlength=17 value="">
						  (同上)</td>
						</tr>
						<tr id="wds_mac_list_3" name="wds_mac_list_3" style="visibility:visible; display:none">
						  <td class="contentpadding" id="basicWDSAPMacAddr">AP MAC 地址:</td>
						  <td><input type=text name=wds_3 size=20 maxlength=17 value="">
					      (同上)</td>
						</tr>
						<tr id="wds_mac_list_4" name="wds_mac_list_4" style="visibility:visible; display:none">
						  <td class="contentpadding" id="basicWDSAPMacAddr">AP MAC 地址:</td>
						  <td><input type=text name=wds_4 size=20 maxlength=17 value="">
					      (同上)</td>
            	...
            	<td><!--<table width="98%" id="div_11n" name="div_11n" border="0" cellspacing="0" cellpadding="0" style="display:none">-->
			  		<table class="space" width="100%" id="div_11n" name="div_11n" border="0" cellspacing="0" cellpadding="0">
						<tr>
						  ...
						  <td width="75%"><input type=radio name=n_mode value="0" checked>混合模式&nbsp;<input type=radio name=n_mode value="1">Green Field</td>
						</tr>
						<tr>
						...
						<tr>
						  ...
						  <td><input type=radio name=n_gi value="0" checked>长&nbsp;<input type=radio name=n_gi value="1">自动</td>
						</tr>
						<tr>
			...
			<tr style="display:none; visibility:hidden">
          	...
          	<tr>
            	<td class="contentpadding"><input type="hidden" value="/wir_wds.asp" name="submit-url"><input type="submit" class="button" value="提 交" />&nbsp;&nbsp;<input type="reset" class="button" value="取 消" onClick="window.location.reload()"></td>
          	</tr>
		  	<tr>
           	   <td>&nbsp;</td>
          	</tr>
