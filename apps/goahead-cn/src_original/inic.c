 ...
 *
 *	$Id: //WIFI_SOC/release/SDK_4_1_0_0/source/user/goahead/src/inic.c#1 $
 */
...
static int	getRaixGBand(int eid, webs_t wp, int argc, char_t **argv);
static int	getRaixABand(int eid, webs_t wp, int argc, char_t **argv);
static int	getRaixAC(int eid, webs_t wp, int argc, char_t **argv);
...
static int  getRaiXM2UBuilt(int eid, webs_t wp, int argc, char_t **argv);
static int  getRaiXTxBfBuilt(int eid, webs_t wp, int argc, char_t **argv);
...
void formDefineInic(void)
{
	...
	websAspDefine(T("getRaixGBand"), getRaixGBand);
	websAspDefine(T("getRaixABand"), getRaixABand);
	websAspDefine(T("getRaixAC"), getRaixAC);
	websAspDefine(T("getRaix11nDraft3Built"), getRaix11nDraft3Built);
	websAspDefine(T("getRaixWAPIBuilt"), getRaixWAPIBuilt);
	websAspDefine(T("getRaiXM2UBuilt"), getRaiXM2UBuilt);
	websAspDefine(T("getRaiXTxBfBuilt"), getRaiXTxBfBuilt);
...
static void inicBasic(webs_t wp, char_t *path, char_t *query)
{
	...
	char_t	*n_mode, *n_bandwidth, *vht_bandwidth, *n_gi, *n_mcs, *n_rdg, *n_extcha, *n_amsdu, *n_stbc;
...
//#WPS
	...
	if (is_n | is_ac) {
		...
		nvram_bufset(RTDEV_NVRAM, "VHT_BW", vht_bandwidth);
...
#if defined (RTDEV_TXBF_SUPPORT)
	if (!strncmp(txbf, "3", 2)) {
		nvram_bufset(RTDEV_NVRAM, "ETxBfEnCond", "1");
		nvram_bufset(RTDEV_NVRAM, "ETxBfIncapable", "0");
		nvram_bufset(RTDEV_NVRAM, "ITxBfEn", "1");
	}
	else if (!strncmp(txbf, "2", 2)) {
		nvram_bufset(RTDEV_NVRAM, "ETxBfEnCond", "1");
		nvram_bufset(RTDEV_NVRAM, "ETxBfIncapable", "0");
		nvram_bufset(RTDEV_NVRAM, "ITxBfEn", "0");
	}
	else if (!strncmp(txbf, "1", 2)) {
		nvram_bufset(RTDEV_NVRAM, "ETxBfEnCond", "0");
		nvram_bufset(RTDEV_NVRAM, "ETxBfIncapable", "1");
		nvram_bufset(RTDEV_NVRAM, "ITxBfEn", "1");
	}
	else {
		nvram_bufset(RTDEV_NVRAM, "ETxBfEnCond", "0");
...
#if defined (RTDEV_IGMPSNOOP_SUPPORT)
	nvram_bufset(RTDEV_NVRAM, "M2UEnabled", m2u_enable);
#endif
