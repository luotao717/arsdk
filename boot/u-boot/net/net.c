...
#else
       // setenv("ethact", "eth0");
       setenv("ethact", "eth1");//by luotao for force the active eth is 1
#endif
	eth_set_current();
#endif
	if (eth_init(bd) < 0) {
...
int autoUpgradeTftp(void) 
{
	unsigned char buf[128]={0};
	unsigned char* nvramEeprom=NULL;
	unsigned char* tempPtr=NULL;
	unsigned char eepromBuf[64]={0};
	strcpy(eepromBuf,"heoc;autook");
	tempPtr=(unsigned char*)0x80080000;
	nvramEeprom=ag7240_env_nvram();
	printf("\neermon==%c-%c-%c-%c\n",*nvramEeprom,*(nvramEeprom+1),*(nvramEeprom+2),*(nvramEeprom+3));
	eth_try_eth1();//force
	sprintf(buf,"tftp %s %s","0x80060000","stb_kf.bin");
	run_command(buf, 0);
	sprintf(buf,"erase 0x9f010000 +0x%s",getenv ("filesize"));
	run_command(buf, 0);
	sprintf(buf,"cp.b %s 0x9f010000 0x%s","0x80060000",getenv ("filesize"));
	run_command(buf, 0);
	memcpy(tempPtr,eepromBuf,64);
	sprintf(buf,"cp.b %s 0x9f3e0000 0x%s","0x80080000","40");
	run_command(buf, 0);
	udelay(1000 * 100);
	run_command("boot", 0);
	//do_reset(0, 0, 0, 0);
...
int autoUpgradeTftpWithCheck(void) 
{
	unsigned char buf[128]={0};
	unsigned char* nvramEeprom=NULL;
	unsigned char* tempPtr=NULL;
	unsigned char eepromBuf[64]={0};
	strcpy(eepromBuf,"heoc;autookhhhhh");
	tempPtr=(unsigned char*)0x80080000;
	nvramEeprom=ag7240_env_nvram();
	printf("\neermoncheck=%c-%c-%c-%c\n",*nvramEeprom,*(nvramEeprom+1),*(nvramEeprom+2),*(nvramEeprom+3));
	if(*(nvramEeprom+5)=='a' && *(nvramEeprom+6) == 'u' && *(nvramEeprom+7) == 't' && *(nvramEeprom+8) == 'o' && *(nvramEeprom+9) == 'o' && *(nvramEeprom+10) == 'k')
	{
		return 0;
	}
	else
